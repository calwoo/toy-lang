...
let eval_primitive (prim : string) : (value list -> value) =
  match prim with
  | "+" -> fun args -> List.fold_left ~f:(fun a b ->
                                        (match (a, b) with
                                        | (ValInt x, ValInt i) -> ValInt (x + i)
                                        | _ -> raise (Bad_interp "can only add numbers"))) ~init:(ValInt 0) args
  | "*" -> fun args -> List.fold_left ~f:(fun a b ->
                                        (match (a, b) with
                                        | (ValInt x, ValInt i) -> ValInt (x * i)
...
let rec eval exp env =
  ...
  | ExprIdent s -> 
      (try ValPrim (eval_primitive s) with
        Bad_interp(_) -> raise (Bad_interp "we only accept builtins now"))
  | ExprFuncAppl(f, args) ->
